...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 19 17:39:44 2024
        ...
        for s in ["x","y"]:
            ...
            for symm in normal_forms.keys():
                if otot in normal_forms[symm].keys():
                    ...
                    Is[symm][matel.ind1,matel.ind2]+=-1.*J
                    Is[symm][matel.ind2,matel.ind1]+=-1.*J
                    break
...
def get_spinless_npa_2(normal_forms, J, U,alpha,shapes,L, PB=False):
    """
    Transverse field ising model Hamiltonian H=\sum_i J(S^{x}_i_S^{x}_{i+1}+S^{y}_i_S^{y}_{i+1})+ Delta S^{z}_i_S^{z}_{i+1}
