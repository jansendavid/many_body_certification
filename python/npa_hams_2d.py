...
def get_xxz2d_npa(normal_forms, J, Delta,shapes,Ls, PB=False):
    """
    ...
    if PB:
        endx=Ls[0]
        endy=Ls[1]
    for i in np.arange(0,endx,1):
        for j in np.arange(0,endy,1):        
            for s in sym:          
                o_1=spin_class_2d.spin_op_2d(s,[min(i, (i+1)%Ls[0]),j],  Lx=Ls[0])
                o_2=spin_class_2d.spin_op_2d(s,[max(i, (i+1)%Ls[0]),j],  Lx=Ls[0])
                ...
                for symmetrie in normal_forms.keys():
                    if otot in normal_forms[symmetrie].keys() and add_term:
                        ...
                        break
                o_1=spin_class_2d.spin_op_2d(s,[i,min(j, (j+1)%Ls[1])],  Lx=Ls[0])
                o_2=spin_class_2d.spin_op_2d(s,[i,max(j, (j+1)%Ls[1])],  Lx=Ls[0])
                otot=spin_class.op_list([o_1,o_2])
                add_term=True
                for symmetrie in normal_forms.keys():
                    if otot in normal_forms[symmetrie].keys() and add_term:
    ...
    for k in Is.keys():
        ...
        Is[k]=0.5*Is[k]
    return Is
def get_tfi2d_npa(normal_forms, J, h,shapes,Ls, PB=False):
    """
    ...
    if PB:
        end_x=Ls[0]
        end_y=Ls[1]
    for i in np.arange(0,Ls[0],1):
        for j in np.arange(0,Ls[1],1):
            s="x"
            o=spin_class_2d.spin_op_2d(s,[i,j], Lx=Ls[0])
    ...
    for i in np.arange(0,end_x,1):
        for j in np.arange(0,end_y,1):
            s="z"
            o_1=spin_class_2d.spin_op_2d(s,[min(i, (i+1)%Ls[0]), j], Lx=Ls[0])
            o_2=spin_class_2d.spin_op_2d(s,[max(i, (i+1)%Ls[0]), j], Lx=Ls[0])
            otot=spin_class.op_list([o_1,o_2])
            ...
            o_1=spin_class_2d.spin_op_2d(s,[i, min(j, (j+1)%Ls[1])], Lx=Ls[0])
            o_2=spin_class_2d.spin_op_2d(s,[i, min(j, (j+1)%Ls[1])], Lx=Ls[0])
            otot=spin_class.op_list([o_1,o_2])
            for symm in normal_forms.keys():
                if otot in normal_forms[symm].keys():
                    matel=normal_forms[symm][spin_class.op_list([o_1,o_2])]
