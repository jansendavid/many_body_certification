...
public:
  ...
  block_shifts.push_back({});
      ...
      for(int i=0; i<L_; i++)
	{
	  std::string block_name="X"+std::to_string(i)+"_"+std::to_string(j)+"_"+sector_label_;
	  blocks_[j].push_back(M_->variable(block_name, Domain::inPSDCone(2*dim_x)));
  ...
  void initialize_blocks_zero(Variable::t variables)
  {
    ...
    for(int i=1; i<L_; i++)
    {
       std::string block_name="bX0_"+sector_label_+"_"+std::to_string(i);
  block_shifts[0].push_back(dim_x);
blocks_[0].push_back(M_->variable(block_name, Domain::inPSDCone(2*(dim_x))));
    }
  ...
  M_->constraint( variables->index(el), Domain::equalsTo(0.0));
    ...
    {
	  std::string block_name="X"+std::to_string(i)+"_"+std::to_string(j)+"_"+sector_label_;
	  blocks_[i].push_back(M_->variable(block_name, Domain::inPSDCone(2*dim_x)));
  ...
  M_->constraint(Expr::add(Expr::sum(Expr::mulDiag(blocks_[0][0],Beta_i)),Expr::mul(-1.*fac.real()*std::sqrt(L_),variables->index(el))), Domain::equalsTo(0.));
    ...
    for(auto it1=operators_.begin(); it1!=operators_.end(); ++it1)
	 ...
	 for(auto it2=it1; it2!=operators_.end(); ++it2)
     {
       ...
       std::string g_key_2="G_"+sector_label_+"_"+std::to_string(j)+"/"+std::to_string(i);
             ...
             for(int mat_pos_y=0; mat_pos_y<L_; mat_pos_y++)
			    {
			       ...
			       auto expr_real_1=Expr::constTerm(0.);
			       auto expr_imag_1=Expr::constTerm(0.);
            ...
            {	      
            double var_real_x=FT_(pos_y,mat_pos_y).real();
			       double var_imag_x=FT_(pos_y,mat_pos_y).imag();
            auto expr_real_2=Expr::constTerm(0.);
			       auto expr_imag_2=Expr::constTerm(0.);
			      for(int pos_x=0; pos_x<L_; pos_x++)
			    {
            	    		       double var_real_x=FT_(pos_x,mat_pos_x).real();
			       double var_imag_x=FT_(pos_x,mat_pos_x).imag();
             // to do, correct so that all terms appearing here appear in map
             auto construct= generate_single_G_element_y(*it1, *it2, g_key,pos_y, pos_x);

			       expr_real_2=Expr::add(expr_real_2, Expr::mul(var_real_x*construct.first.prefac_,variables->index(construct.first.pos_) ));
			       expr_real_2=Expr::add(expr_real_2,  Expr::mul(-1.*var_imag_x*construct.second.prefac_,variables->index(construct.second.pos_))) ;

			       expr_imag_2=Expr::add(expr_imag_2,Expr::mul(var_imag_x*construct.first.prefac_,variables->index(construct.first.pos_) ) );
			        expr_imag_2=Expr::add(expr_imag_2,Expr::mul(var_real_x*construct.second.prefac_,variables->index(construct.second.pos_) ));

          }
            			       double var_real_y=FT_(pos_y,mat_pos_y).real();
			       double var_imag_y=FT_(pos_y,mat_pos_y).imag();
             // to do, correct so that all terms appearing here appear in map
			       expr_real_1=Expr::add(expr_real_1, Expr::mul(var_real_x,expr_real_2 ));
			       expr_real_1=Expr::add(expr_real_1,  Expr::mul(-1.*var_imag_x,expr_imag_2)) ;

			       expr_imag_1=Expr::add(expr_imag_1,Expr::mul(var_imag_x,expr_real_2 ) );
			        expr_imag_1=Expr::add(expr_imag_1,Expr::mul(var_real_x,expr_imag_2));
...
// 			    }
			  ...
			  // real part
        //std::cout<< " dim "<<dim<< " shift "<< shift<< " mat_pos_x "<< mat_pos_x << " mat_pos_y "<< mat_pos_y<<std::endl;
        {std::vector<double> val={1./2,1./2, 1./2, 1./2};
...
//      auto itx=sectors_.begin();
//        while(itx!=sectors_.end())
// 	 {
//     std::cout<< " number of blocks "<<  itx->second.block_shifts.size()<< " and "<<itx->second.blocks_.size()<<std::endl;
//      std::cout<< "sector "<<std::endl;
//      for(int i=0; i<L_; i++)
//      {
//       std::cout<< "size "<< itx->second.block_shifts[i].size()<< " and "<<itx->second.blocks_[i].size()<<std::endl;
// for(int j=0; j<L_; j++)
// {
// std::cout<< " i,j"<< i << " , "<<j<<std::endl;
// std::cout<< "  dim "<< itx->second.block_shifts[i][j]<<std::endl;
// std::cout<<std::endl;
