...
auto X=M->variable("X", Domain::inPSDCone(dim));

auto it=matrix_mapping.begin();
auto C=matrix_mapping[it->first].make_matrix(dim, dim);
//M->constraint( Expr::sum(Expr::mulDiag(X,C )), Domain::equalsTo(b[0]));
it++;
 int l=1;
while(it!=matrix_mapping.end())
{

	M->constraint( Expr::dot(matrix_mapping[it->first].make_matrix(dim, dim),Expr::neg(X) ), Domain::equalsTo(-1*b[l]));
...
l++;
        ...
        M->objective(ObjectiveSense::Maximize, Expr::dot(C,Expr::neg(X)));
