...
std::vector<double> define_ham(std::map<std::string, int> terms_mapping, double J, double Delta, int L)
{
	...
	for (auto s : sym)
	{
		for (int i = 0; i < L; i++)
		{
			op_vec v0 = {spin_op(s, {i}, {1}), spin_op(s, {(i + 1) % L}, {1})};
...
int main()
{
	...
	for (int i = 0; i < L; i++)
	{
		for (auto s : sym)
		{
			op_vec v1 = {spin_op(s, {i}, {1}), spin_op(s, {(i + 1) % L}, {1})};
	...
	for (int i = 0; i < L; i++)
	{
		for (auto &a : occ)
		{
			op_vec v1 = {spin_op(a.first, {i}, {1}), spin_op(a.second, {(i + 1) % L}, {1})};
			auto [fac1, nf1] = get_normal_form(v1);
			v_tot.push_back(nf1);
		}
	}
