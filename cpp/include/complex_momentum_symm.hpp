 ...
 void generate_G_elements(op_vec op1, op_vec op2, std::string key, int i)
  {
    ...
    v_x.insert(v_x.end(), new_op.begin(),new_op.end());
     ...
     //       auto ti_val=TI_map_.at(print_op(vec)).second;
	 ...
	 for(auto op_t: all_t)
	   {
		 ...
		 TI_map_.insert({print_op(op), { it->first,fac}});
		 found=true;
		 break;
	       }    
	   }
...
public:
  ...
  momentum_block(int L,std::vector<op_vec> operators, Model::t M, bool is_zero, TI_map_type& TI_map,std::map<std::string, mom_ref>& total_refs, Eigen::MatrixXcd& FT, std::string sector_label=""):is_zero_(is_zero),  momentum_block_base(L, operators, M,TI_map, total_refs,FT, sector_label){
    ...
    // if(is_zero)
    //   {
    //  initialize_blocks_zero();
    //}
    //    else{initialize_blocks_zero();}
    //  generate_block();
  ...
  void initialize_blocks_zero()
  {
      ...
      for(int i=1; i<L_; i++)
	{
	 ...
	 auto ti_key=TI_map_.at(print_op(op)).first;
	 auto el=total_refs_.at(ti_key);
...
//   //  //      std::cout<<FT<<std::endl;
  ...
  // 			      
   			      double var_real=FT_(mat_pos,pos).real();
			      double var_imag=FT_(mat_pos,pos).imag();
			  ...
			  // imaginary part
			   ...
			   // fix diagonal part of H_I is zero, needed?
...
public:
  ...
  momentum_basis(int L, basis_structure operators,Model::t M):L_(L),operators_(operators), M_(M)
  {
	...
	auto Block=momentum_block(L_,it->second,M_, true,TI_map_,total_refs_, FT_, std::to_string(it->first));
