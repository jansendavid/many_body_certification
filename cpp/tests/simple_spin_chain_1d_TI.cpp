...
//    std::cout << "rdm size "<<rdms.size() << std::endl;
   ...
   auto offset = std::vector<int> {Lx, Ly}; //lattice.get_offset_vec();

//    for (const auto& [k, v] : lattice.TI_map_)
//    {
//        auto lhs = unpack_key(k, offset);
//        auto rhs = unpack_key(v.first, offset);
...
//    }
   ...
   double Delta = 1.;
    ...
    auto b = define_xxz1d_sos(lattice, J, Delta);
