...
int main()
{
    ...
    for (int i = 0; i < L; i++)
    {
        for (auto s1 : sym)
        {
            ...
            states[map_sec.at(sign)].push_back(nf1);
    ...
    for (int i = 0; i < L; i++)
    {
        ...
        for (auto s1 : sym)
        {
            ...
            //if(i!=j)
            {
            op_vec v1 = {spin_op(s1, {i}, {1}),spin_op(s2, {(i+1)%L}, {1})};
            auto [fac1, nf1] = get_normal_form(v1);
            auto sign = get_sec(nf1);
           states[map_sec.at(sign)].push_back(nf1);
            }
...
for (int i = 0; i < L; i++)
{
    ...
    auto mat_terms = get_mat_terms(states);
    int index_old = 0;
    ...
    for (auto symm_sec : mat_terms)
    {
        ...
        for (auto & sec : states)
        {
            ...
            {
                get_sdp_block_general(M, states[sec.first], matrix_mapping[sec.first], elements_mapping, terms_mapping, y);
            }
