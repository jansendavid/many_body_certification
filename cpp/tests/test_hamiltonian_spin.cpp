...
Matrix::t get_ham(  std::unordered_map<std::string, mat_el_ref> mat_map, int L, int dim, double t0, double U)
{
  ...
  std::vector<std::string> spin={"up", "dn"};
  for(int i=0; i<L; i++)
    {
      for(auto s: spin){
       op_vec v1={electron_op(s, {i}, true),electron_op(s, {(i+1)%L}, false)};
       ...
       v1={electron_op(s, {(i+1)%L}, true),electron_op(s, {i}, false)};
...
void fix_particle_number(Model::t& M,int L, int part_nr,std::unordered_map<std::string, mat_el_ref> mat_map, std::string spin="up")
{
  op_vec v1={electron_op(spin, {0}, true),electron_op(spin, {0}, false)};
  auto el=mat_map[print_op(v1)];
  auto expr=Expr::mul(1*el.coeff_,el.var_);
  std::cout<< print_op(v1)<< " "<< el<<std::endl;
  for(int i=1; i<L; i++)
    {
       v1={electron_op(spin, {i}, true),electron_op(spin, {i}, false)};
       el=mat_map[print_op(v1)];
       //std::cout<< print_op(v1)<< " "<< el<<std::endl;
  ...
  for(int i=0; i<L; i++)
    {
      ...
      op_vec v2={electron_op("up", {i}, false)};
       v_tot.push_back(v1);      
       v_tot.push_back(v2);
       v1={electron_op("dn", {i}, true)};
       v2={electron_op("dn", {i}, false)};
       v_tot.push_back(v1);      
       v_tot.push_back(v2);
...
int main()
{
  ...
  //   }
     ...
     std::vector<std::string> extra_terms_vec;
   ...
   for(int j=0; j<v_tot.size(); j++)
	 ...
	 for(int l=0; l<all_terms.size(); l++)
	   {
	     ...
	     extra_terms_vec.push_back(print_op(all_terms[l].second));
     ...
     for(auto a: extra_terms_vec)
       {
	 if (mat_terms.find(a) == mat_terms.end() and extra_terms.find(a) == extra_terms.end() ){
	   extra_terms.insert({a, mat_el(new_ind,v_tot.size()+1, 1)});
	   std::cout<< "ex "<< a<< "  " << new_ind<< std::endl;
	   new_ind++;
	 }
       }
 ...
 // // std::cout<< el.coeff_<< "  "<<all_terms[0].first <<std::endl;
   ...
   // std::cout<< j+1 << "  " << i+1 << " ro be "<<  el<< "  "<<std::endl;
    ...
    fix_particle_number(M, L, part_nr,total_refs, "up");
    fix_particle_number(M, L, part_nr,total_refs, "dn");
